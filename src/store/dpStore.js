...
import { getDPAlgorithm } from '../app/visualizer/algorithms/dp';
  ...
  setInput: (algorithm, newInput) => {
    ...
    if (algorithm === 'knapsack') {
      ...
      if (validatedInput.weights && Array.isArray(validatedInput.weights)) {
        validatedInput.weights = validatedInput.weights.filter(
          val => !isNaN(val) && val >= 0
        );
      }
      if (validatedInput.values && Array.isArray(validatedInput.values)) {
        validatedInput.values = validatedInput.values.filter(
          val => !isNaN(val) && val >= 0
        );
      }
      ...
      if (validatedInput.weights.length !== validatedInput.values.length) {
        ...
        const minLength = Math.min(
          validatedInput.weights.length,
          validatedInput.values.length
        );
  ...
  startAlgorithm: async name => {
    ...
    try {
      ...
      switch (name) {
        ...
        case 'lcs':
          set({
            table: Array(input.lcs.str1.length + 1)
              .fill()
        ...
        case 'knapsack':
          set({
            table: Array(input.knapsack.weights.length + 1)
              .fill()
