...
import { getGraphAlgorithm } from '../app/visualizer/algorithms/graph';
    ...
    switch (layoutType) {
      ...
      case 'grid':
        ...
        switch (gridVariant) {
          ...
          case 'hexagonal':
            const hexRadius =
              Math.min(width, height) / Math.sqrt(nodeCount * 4);
            nodes.forEach((node, i) => {
              const row = Math.floor(i / 5);
              const col = i % 5;
              node.x =
                centerX + col * hexRadius * 1.7 + (row % 2 ? hexRadius : 0);
  ...
  generateGraph: () => {
    ...
    while (edges.length < edgeCount) {
      ...
      while (
        target === source ||
        edges.some(
          e => e.source === nodes[source].id && e.target === nodes[target].id
        )
      ) {
