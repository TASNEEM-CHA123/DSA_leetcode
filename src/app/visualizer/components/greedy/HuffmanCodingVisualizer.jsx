...
const HuffmanCodingVisualizer = () => {
  const { frequencies, huffmanTree, huffmanCodes } = useGreedyStore();
  ...
  const renderHuffmanCodes = () => {
    if (!huffmanCodes || Object.keys(huffmanCodes).length === 0) {
      return (
        <p className="text-gray-400">Codes will appear as the algorithm runs</p>
      );
    }
    ...
    return (
      <g key={nodeId}>
        ...
        <motion.circle
          ...
          className={
            node.char
              ? 'fill-green-500 stroke-white stroke-2'
    ...
    <div className="flex flex-col gap-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-2">
      ...
      <div className="p-4 rounded-lg bg-slate-800">
        ...
        <p className="text-gray-300">
          ...
          Characters that occur more frequently get shorter codes.
        </p>
        <ul className="pl-5 mt-2 text-gray-300 list-disc">
          ...
          <li>
            Extract two nodes with lowest frequencies and create a new internal
            node
          </li>
