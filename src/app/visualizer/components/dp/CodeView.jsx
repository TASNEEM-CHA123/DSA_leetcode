...
const CodeView = ({ algorithm, currentCell, isPlaying }) => {
  ...
  return dp[n];
      ...
      getExplanation: cell => {
        ...
        if (cell <= 1)
          return 'Base case: For n=0 or n=1, we return the value directly.';
      ...
      executionSequence: [
        ...
        {
          lines: [2, 3],
          explanation: 'Check base cases: if n is 0 or 1, return n',
        },
  ...
  return dp[n][capacity];
      ...
      getExplanation: cell => {
        ...
        const [i, w] = cell;
        if (i === 0 || w === 0)
          return 'Base case: Empty knapsack or no items has value 0';
        return w >= i
      ...
      executionSequence: [
        ...
        {
          lines: [10, 11, 12],
          explanation: 'If it fits, decide whether to take or leave it',
        },
  ...
  return Math.max(...dp);
      ...
      getExplanation: cell => {
        if (cell === null) return 'Algorithm not started yet.';
        if (cell === 0)
          return 'Initialize all positions with LIS=1 (single element)';
      ...
      executionSequence: [
        { lines: [1, 2], explanation: 'Start with array of length n' },
        {
          lines: [4],
          explanation:
            'Initialize with 1s - every element is a LIS of length 1',
        },
        {
          lines: [7],
          explanation: 'For each element starting from index 1...',
        },
        { lines: [8], explanation: 'Compare with all previous elements' },
        {
          lines: [9],
          explanation:
            'If current > previous and forms longer sequence, update',
        },
  ...
  return dp[m][n];
      ...
      executionSequence: [
        {
          lines: [1, 2, 3],
          explanation: 'Start with two strings of length m and n',
        },
        {
          lines: [5],
          explanation: 'Initialize (m+1) × (n+1) table with zeros',
        },
        { lines: [8, 9], explanation: 'For each character pair...' },
        { lines: [10], explanation: 'Check if characters match' },
        {
          lines: [11],
          explanation: 'If they match, extend LCS by 1 from previous result',
        },
        {
          lines: [13],
          explanation:
            "If they don't match, take best of excluding one character",
        },
          ...
          onComplete: () => {
            ...
            if (currentCell !== null) {
              const newHighlightedLines =
                codeSnippets[algorithm].getHighlightedLines(currentCell);
              setHighlightedLines(newHighlightedLines);
              setExplanation(
                codeSnippets[algorithm].getExplanation(currentCell)
              );
            }
          },
        },
  ...
  if (!algorithm || !codeSnippets[algorithm]) {
    return (
      <div className="text-white">No code available for this algorithm</div>
    );
  }
