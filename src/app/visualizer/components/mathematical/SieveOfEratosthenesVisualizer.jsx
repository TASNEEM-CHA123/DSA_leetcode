...
const SieveOfEratosthenesVisualizer = ({
  maxNumber,
  isAnimating,
  speed,
  onComplete,
  ...
  useEffect(() => {
    ...
    const runSieveStep = () => {
      ...
      setMessage(
        `Found prime number: ${nextPrime}. Now marking all multiples as non-prime.`
      );
    ...
    const markMultiples = prime => {
      ...
      if (multipleIndex !== -1) {
        ...
        setMessage(
          `Marking ${currentMultipleVal} (multiple of ${prime}) as non-prime`
        );
      ...
      for (let i = currentMultipleVal + prime; i <= maxNumber; i += prime) {
        const idx = updatedGrid.findIndex(
          item => item.number === i && item.isPrime
        );
        if (idx !== -1) {
          allMarked = false;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        className="flex-1 flex flex-col md:flex-row items-center justify-center p-6 bg-slate-900 rounded-lg overflow-auto"
        ref={gridRef}
      >
        <div
          ...
          >
            ...
            <div className="mt-2 flex flex-col md:flex-row md:flex-wrap md:justify-center gap-2 primes-list">
              {primes.map(prime => (
                <span
                  key={prime}
                  className="px-2 py-1 bg-green-900 rounded-md text-green-100"
                >
                  {prime}
                </span>
              ))}
