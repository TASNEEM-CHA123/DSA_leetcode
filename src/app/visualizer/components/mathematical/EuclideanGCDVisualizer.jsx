...
const EuclideanGCDVisualizer = ({
  ...
  useEffect(() => {
    if (num1 > 0 && num2 > 0) {
      ...
      setSteps(newSteps);
      setResult(
        newSteps[newSteps.length - 1].remainder === 0
          ? newSteps[newSteps.length - 1].divisor
          : null
      );
      setCurrentStep(0);
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              {currentStep === index && isAnimating && (
                <span className="text-xs px-2 py-1 bg-sky-600 rounded-full text-white">
                  Current
                </span>
              )}
              {currentStep > index && (
          ...
          {currentStep < steps.length && (
            <div className="flex flex-col items-center">
              <div className="mb-8 flex flex-wrap max-w-md justify-center">
                ...
                {steps[currentStep].dividend > 0 &&
                  steps[currentStep].divisor > 0 &&
                  Array.from({
                    length: Math.min(steps[currentStep].divisor, 50),
                  }).map((_, i) => (
                    <motion.div
                      key={`rect-${i}`}
              ...
              <div className="text-center text-white">
                <div className="mb-2 text-xl">
                  {steps[currentStep].dividend} = {steps[currentStep].divisor} ×{' '}
                  {steps[currentStep].quotient} + {steps[currentStep].remainder}
                </div>
