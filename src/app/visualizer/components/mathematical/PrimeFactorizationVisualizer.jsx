...
const PrimeFactorizationVisualizer = ({
  number,
  isAnimating,
  speed,
  onComplete,
  ...
  useEffect(() => {
    ...
    const runAnimation = () => {
      if (stepIndex >= steps.length) {
        ...
        setMessage(
          `Factorization complete! ${number} = ${factors.join(' × ')}`
        );
      ...
      if (step.isDivisible) {
        setMessage(
          `${step.number} is divisible by ${step.divisor}: ${step.number} ÷ ${step.divisor} = ${step.quotient}`
        );
      } else {
        setMessage(
          `${step.number} is not divisible by ${step.divisor}, trying next divisor`
        );
      }
  ...
  const calculateTreeLayout = factors => {
    ...
    while (processedFactors < factors.length) {
      ...
      for (
        let i = 0;
        i < nodesInCurrentLevel && processedFactors < factors.length;
        i++
      ) {
        ...
        nodes.push({
          ...
          parentId:
            currentLevel === 1
              ? 'root'
              : `node-${Math.floor((processedFactors - 1) / 2)}`,
        });
      ...
      currentLevel++;
      nodesInCurrentLevel = Math.min(
        nodesInCurrentLevel * 2,
        factors.length - processedFactors
      );
  ...
  const handleWheel = e => {
    ...
    const newScale = Math.max(
      0.5,
      Math.min(3, transform.scale + (delta > 0 ? scaleFactor : -scaleFactor))
    );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
        {/* Left side - Steps */}
        <div
          className="p-4 bg-slate-900 rounded-lg overflow-auto h-[45%]"
        ...
        >
          ...
          {steps.length > 0 ? (
            ...
            <div className="text-center text-gray-400 py-8">
              Enter a number greater than 1 to see factorization steps
            </div>
          )}
                  ...
                  >
                        ...
                        return (
                          <motion.line
                            ...
                            animate={{
                              ...
                              opacity:
                                animationCompleted ||
                                factors.indexOf(node.value) < currentStep
                                  ? 1
                                  : 0.3,
                            }}
                            transition={{
                    ...
                    {treeNodes
                      ...
                      .map((node, i) => {
                        ...
                        const isVisible =
                          animationCompleted ||
                          factors.indexOf(factor) < currentStep;
