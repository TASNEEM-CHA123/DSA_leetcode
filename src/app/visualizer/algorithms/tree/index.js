...
export const treeTraversals = async (root, onStep) => {
  ...
  await Promise.all([inorder(root), preorder(root), postorder(root)]);
...
export const avlRotations = {
  ...
  getBalance: node => {
    return node
      ? avlRotations.getHeight(node.left) - avlRotations.getHeight(node.right)
      : 0;
  },
  ...
  rightRotate: async (y, onStep) => {
    ...
    y.height =
      Math.max(
        avlRotations.getHeight(y.left),
        avlRotations.getHeight(y.right)
      ) + 1;
    x.height =
      Math.max(
        avlRotations.getHeight(x.left),
        avlRotations.getHeight(x.right)
      ) + 1;
